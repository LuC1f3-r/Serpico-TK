...
class BaseWindow:
    ...
    def set_icon(self):
        if 'win32' in sys.platform:
            ...
            img_ = tk.Image('photo', file=os.path.join(
                config.CODEPATH, 'res', 'icon3.png'))
        ...
        if dialog and os.path.isfile(dialog):
            ...
            if fsize and (size_ != fsize):
                raise FileHandlerError(
                    f'The file selected is {size_} bytes, but {fsize} is expected.')
            if fsizes and (size_ not in fsizes):
                raise FileHandlerError(
                    'The file selected is of unexpected size.')
