...
class MainWindow(BaseWindow):
    ...
    def __init__(self, **kwargs):
        ...
        createToolTip(self.output_button,
                      'Select the output location where data will be saved to')
        ...
        createToolTip(self.check_button,
        ...
        createToolTip(self.extract_button,
        ...
        self.force_backup_button.grid(
            row=4, column=0, columnspan=2, sticky=tk.W)
        createToolTip(self.force_backup_button,
        ...
        self.extract_shared_button.grid(
            row=5, column=0, columnspan=2, sticky=tk.W)
        createToolTip(self.extract_shared_button,
        ...
        createToolTip(
            self.extract_tar, "Choose the 'data.tar' backup file to be parsed and data decoded")
        ...
        save_log = ttk.Button(
            self.mainframe, text='Save Log', command=self.save_log)
        ...
        self.logger.info(
            f"Time in reports:  {self.time_now_configured} <--")  # \u2190
    ...
    def build_file_menus(self):
        ...
        menu_file.add_command(
            label='Save log', underline=0, command=self.save_log)
        menu_file.add_command(
            label='Clear log', underline=0, command=self.clear_log)
        menu_file.add_command(label='Preferences', command=self.preferences)
        menu_file.add_separator()
        menu_file.add_command(label='Exit', underline=1,
                              command=self.root.destroy, accelerator='Esc * 2')
    ...
    def build_decoders_menus(self):
        ...
        for decoder in sorted(self.registry.decoders, key=lambda d: d.__name__):
            ...
            def action(x=dec): return self.decode_file(x)
    ...
    def build_help_menus(self):
        ...
        if not getattr(sys, 'frozen', False):
            menu_help.add_command(
                label='Run Update', command=lambda: self.conf.upgrade_package(logger=self.logger))
            menu_help.add_separator()
    ...
    def build_tools_menus(self):
        ...
        menu_tools.add_command(
            label='Convert AB to TAR file..', command=self.ab_to_tar)
        menu_tools.add_command(
